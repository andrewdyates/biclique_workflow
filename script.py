...
NOTE: Support is in % percent!
  python $HOME/pymod/biclique_workflow/script.py depM_fname=/fs/lustre/osu6683/gse15745_nov2/dependency_dispatch/Methyl-correct-aligned.pkl_mRNA-correct-aligned.pkl.DCOR.values.pkl support=1 threshold=0.5 density=0.4 work_dir=/fs/lustre/osu6683/gse15745_nov2/bicliques thresh_cmp=greater
...
def workflow(depM_fname=None, work_dir=None, support=10.0, threshold=0.6, thresh_cmp="greater", absvalue=False, add_mpiexec=False, overwrite=False, density=0.7, merge_type=1, extract_rows=None, extract_cols=None):
  ...
  elif os.path.exists(work_dir) and not os.path.isdir(work_dir):
    print "WARNING: %s is an existing file. Choose another work_dir path. Exiting..." % (work_dir)
    sys.exit(1)
  else:
    make_dir(work_dir)
  ...
  # Be sure to remove duplicates and sort extraction lists!
